...
def ensure_syft_yaml(client):
    ...
    if syft_yaml_path.exists():
        with syft_yaml_path.open('r') as file:
    ...
    with syft_yaml_path.open('w') as file:
    ...
    if not datasets_yaml_path.exists():
        with datasets_yaml_path.open('w') as file:
            file.write('---\n')  # Add the YAML document start marker
...
def add_dataset(client, dataset_name, dataset_path, schema_name):
    ...
    if datasets_yaml_path.exists():
        with datasets_yaml_path.open('r') as file:
            loaded = yaml.safe_load(file) or {}
            # Merge in a way that ensures required keys
            datasets_data.update({k: loaded.get(k, v) for k, v in datasets_data.items()})
    ...
    with schema_file_path.open('w') as schema_file:
        schema_file.write('---\n')  # Add the YAML document start marker
    ...
    existing_resource = next((res for res in datasets_data['resources'] if res['name'] == dataset_name), None)
